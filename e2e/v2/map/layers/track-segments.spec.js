...
test.describe("Track Transportation Modes", () => {
  ...
  async function enableTracksLayerOnly(page) {
    ...
    const tracksCheckbox = page.locator('label:has-text("Tracks") input.toggle').first();
    ...
    const layersToDisable = ["Routes", "Areas", "Visits", "Places", "Photos", "Heatmap", "Points"];
    for (const layer of layersToDisable) {
      const checkbox = page.locator(`label:has-text("${layer}") input.toggle`).first();
      ...
      if (trackId) {
        ...
        if (count > 0) {
          ...
          const hasTimePattern =
            /\d{2}:\d{2}/.test(firstItemText) ||
            firstItemText.includes("--:--");
  ...
  test.describe("Clearing Track Selection", () => {
    test("clicking elsewhere on map clears track selection", async ({
      ...
      if (trackId) {
        ...
        const segmentLayerVisible = await page.evaluate(() => {
          const element = document.querySelector(
            '[data-controller*="maps--maplibre"]',
          );
          if (!element) return false;
