  ...
  test.describe('Dragging', () => {
    test('allows dragging points to new positions', async ({ page }) => {
      ...
      const dragResult = await page.evaluate(async () => {
        ...
        if (!pointsLayer || !map) return { success: false, reason: 'no layer or map' }

        const source = map.getSource('points-source')
        const data = source?._data
        if (!data?.features?.length) return { success: false, reason: 'no features' }
        ...
        const draggedFeature = pointsLayer.draggedFeature
        pointsLayer.draggedFeature = null

        // Make the API call to persist the change
        try {
          const apiKeyEl = document.querySelector('[data-maps--maplibre-api-key-value]')
          const apiKey = apiKeyEl?.getAttribute('data-maps--maplibre-api-key-value')
          if (apiKey) {
      ...
      const updatedPoint = updatedData.features.find(f => f.properties.id === dragResult.pointId)
    ...
    test('updates connected route segments when point is dragged', async ({ page }) => {
        ...
        if (routesSource?._data?.features) {
          ...
          routesData.features.forEach(route => {
            route.geometry.coordinates = route.geometry.coordinates.map(coord => {
              if (Math.abs(coord[0] - originalCoords[0]) < 0.0001 &&
      ...
      const updatedPoint = updatedPointsData.features.find(f => f.properties.id === dragResult.pointId)
      expect(updatedPoint).toBeDefined()
      const updatedCoords = updatedPoint.geometry.coordinates
    ...
    test('persists point position after page reload', async ({ page }) => {
      ...
      const afterDragPoint = afterDragData.features.find(f => f.properties.id === dragResult.pointId)
      ...
      const afterReloadPoint = afterReloadData.features.find(f => f.properties.id === dragResult.pointId)
