...
import { waitForMap, openSettingsPanel } from '../helpers/map.js';
...
test.describe('Settings Panel', () => {
  ...
  test('should close settings panel when clicking gear again', async ({ page }) => {
    // Open
    await openSettingsPanel(page);
    await expect(page.locator('.leaflet-settings-panel')).toBeVisible();
  ...
  test('should show correct default values from user settings', async ({ page }) => {
    await openSettingsPanel(page);
  ...
  test('should have Update button', async ({ page }) => {
    ...
    const submitButton = page.locator('#settings-form button[type="submit"], #settings-form input[type="submit"]');
