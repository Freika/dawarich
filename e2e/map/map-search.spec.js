...
test.describe('Map Search', () => {
  ...
  test('should open search bar when clicking search button', async ({ page }) => {
  ...
  test('should close search bar when clicking close button', async ({ page }) => {
    // Open
    await page.locator('#location-search-toggle').click();
    await page.waitForTimeout(300);
    ...
    await expect(page.locator('#location-search-container')).toHaveClass(/hidden/);
  });
  ...
  test('should show search input with placeholder', async ({ page }) => {
    ...
    const placeholder = await page.locator('#location-search-input').getAttribute('placeholder');
  ...
  test('should have results panels initially hidden', async ({ page }) => {
    ...
    await expect(page.locator('#location-search-suggestions-panel')).toHaveClass(/hidden/);
    await expect(page.locator('#location-search-results-panel')).toHaveClass(/hidden/);
  });
});
