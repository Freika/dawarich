...
ActiveRecord::Schema[8.0].define(version: 20_260_326_152_648) do
...
unique: true
  ...
  create_table 'digests', force: :cascade do |t|
    ...
    t.index %w[user_id year month period_type], name: 'index_digests_on_user_year_month_period_type',
unique: true
  ...
  create_table 'family_invitations', force: :cascade do |t|
    ...
    t.datetime 'created_at', null: false
    t.datetime 'updated_at', null: false
    t.index %w[family_id email], name: 'index_family_invitations_on_family_id_and_email'
    t.index %w[family_id status expires_at], name: 'index_family_invitations_on_family_status_expires'
    t.index %w[status expires_at], name: 'index_family_invitations_on_status_and_expires_at'
    t.index %w[status updated_at], name: 'index_family_invitations_on_status_and_updated_at'
  ...
  create_table 'family_location_requests', force: :cascade do |t|
    ...
    t.datetime 'created_at', null: false
    t.datetime 'updated_at', null: false
    t.index %w[expires_at status], name: 'idx_family_loc_requests_expires_status'
    t.index ['family_id'], name: 'index_family_location_requests_on_family_id'
    t.index %w[requester_id target_user_id status], name: 'idx_family_loc_requests_requester_target_status'
    t.index %w[target_user_id status], name: 'idx_family_loc_requests_target_status'
  end
  ...
  create_table 'family_memberships', force: :cascade do |t|
    ...
    t.datetime 'created_at', null: false
    t.datetime 'updated_at', null: false
    t.index %w[family_id role], name: 'index_family_memberships_on_family_and_role'
  ...
  create_table 'place_visits', force: :cascade do |t|
    ...
    t.index %w[visit_id place_id], name: 'idx_place_visits_visit_id_place_id', unique: true
  end
  ...
  create_table 'points', force: :cascade do |t|
    ...
    t.index %w[track_id timestamp], name: 'idx_points_track_id_timestamp'
    t.index ['track_id'], name: 'index_points_on_track_id'
    t.index %w[user_id country_name], name: 'idx_points_user_country_name'
    t.index %w[user_id geodata], name: 'index_points_on_user_id_and_empty_geodata', where: "(geodata = '{}'::jsonb)"
    t.index %w[user_id id], name: 'index_points_on_unarchived',
where: "((raw_data_archived = false) AND (raw_data <> '{}'::jsonb))"
    t.index %w[user_id timestamp track_id], name: 'idx_points_track_generation'
    t.index %w[user_id timestamp], name: 'idx_points_user_visit_null_timestamp', where: '(visit_id IS NULL)'
    t.index %w[user_id timestamp], name: 'index_points_on_user_id_and_timestamp', order: { timestamp: :desc }
  ...
  create_table 'points_raw_data_archives', force: :cascade do |t|
    ...
    t.index %w[user_id year month chunk_number], name: 'index_raw_data_archives_uniqueness', unique: true
    t.index %w[user_id year month], name: 'index_points_raw_data_archives_on_user_id_and_year_and_month'
  ...
  create_table 'rails_pulse_operations', force: :cascade do |t|
    ...
    t.datetime 'created_at', null: false
    t.datetime 'updated_at', null: false
    t.index %w[created_at query_id], name: 'idx_operations_for_aggregation'
    t.index ['created_at'], name: 'idx_operations_created_at'
    t.index %w[occurred_at duration operation_type], name: 'index_rails_pulse_operations_on_time_duration_type'
    ...
    t.index %w[query_id duration occurred_at], name: 'index_rails_pulse_operations_query_performance'
    t.index %w[query_id occurred_at], name: 'index_rails_pulse_operations_on_query_and_time'
...
comment: 'Normalized SQL query string (e.g., SELECT * FROM users WHERE id = ?)'
  ...
  create_table 'rails_pulse_requests', force: :cascade do |t|
    ...
    t.datetime 'created_at', null: false
    t.datetime 'updated_at', null: false
    t.index %w[created_at route_id], name: 'idx_requests_for_aggregation'
    ...
    t.index %w[route_id occurred_at], name: 'index_rails_pulse_requests_on_route_id_and_occurred_at'
  ...
  create_table 'rails_pulse_routes', force: :cascade do |t|
    ...
    t.datetime 'created_at', null: false
    t.datetime 'updated_at', null: false
    t.index %w[method path], name: 'index_rails_pulse_routes_on_method_and_path', unique: true
  end
  ...
  create_table 'rails_pulse_summaries', force: :cascade do |t|
    ...
    t.index %w[period_type period_start], name: 'index_rails_pulse_summaries_on_period'
    t.index %w[summarizable_type summarizable_id period_type period_start],
            name: 'idx_pulse_summaries_unique', unique: true
    t.index %w[summarizable_type summarizable_id], name: 'index_rails_pulse_summaries_on_summarizable'
  end
  ...
  create_table 'stats', force: :cascade do |t|
    ...
    t.index ['h3_hex_ids'], name: 'index_stats_on_h3_hex_ids',
where: "((h3_hex_ids IS NOT NULL) AND (h3_hex_ids <> '{}'::jsonb))", using: :gin
    t.index ['month'], name: 'index_stats_on_month'
    t.index ['sharing_uuid'], name: 'index_stats_on_sharing_uuid', unique: true
    t.index %w[user_id year month], name: 'index_stats_on_user_id_year_month', unique: true
  ...
  create_table 'taggings', force: :cascade do |t|
    ...
    t.index %w[taggable_type taggable_id tag_id], name: 'index_taggings_on_taggable_and_tag', unique: true
  ...
  create_table 'tags', force: :cascade do |t|
    ...
    t.index ['privacy_radius_meters'], name: 'index_tags_on_privacy_radius_meters',
where: '(privacy_radius_meters IS NOT NULL)'
  ...
  create_table 'track_segments', force: :cascade do |t|
    ...
    t.datetime 'created_at', null: false
    t.datetime 'updated_at', null: false
    t.index %w[track_id start_index end_index], name: 'index_track_segments_on_track_and_indices'
    t.index %w[track_id transportation_mode], name: 'index_track_segments_on_track_id_and_transportation_mode'
  end
  ...
  create_table 'tracks', force: :cascade do |t|
    ...
    t.index %w[user_id start_at], name: 'idx_tracks_user_id_start_at'
...
'minutes_between_routes' => '60' }
  ...
  create_table 'video_exports', force: :cascade do |t|
    ...
    t.index %w[user_id status], name: 'index_video_exports_on_user_id_and_status'
