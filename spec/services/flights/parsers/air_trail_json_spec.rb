...
RSpec.describe Flights::Parsers::AirTrailJson do
  ...
  it 'assigns a stable synthetic id when missing' do
    flights = described_class.call({ 'flights' => [export_flight] })

    id1 = flights.first['id']
    id2 = described_class.call({ 'flights' => [export_flight] }).first['id']
  ...
  it 'preserves an existing AirTrail id' do
    flights = described_class.call({ 'flights' => [export_flight('id' => 42)] })

    expect(flights.first['id']).to eq(42)
  end
