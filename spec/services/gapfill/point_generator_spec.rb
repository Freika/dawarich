...
RSpec.describe Gapfill::PointGenerator do
  ...
  let(:start_point) do
    create(:point, user: user, lonlat: 'POINT(13.3888 52.5170)', timestamp: 1_000_000, tracker_id: 'demo')
  end
  let(:end_point) do
    create(:point, user: user, lonlat: 'POINT(13.4050 52.5200)', timestamp: 1_001_000, tracker_id: 'demo')
  end
  ...
  describe '#build_points' do
    ...
    it 'inherits tracker_id from the start point so inferred points render in the same route bucket' do
      points = generator.build_points
      expect(points).to all(have_attributes(tracker_id: 'demo'))
    end
