...
RSpec.describe 'Simplified Chinese locale coverage' do
  ...
  let(:chinese) { flatten(locale_tree('zh')) }
  ...
  it 'contains every English locale leaf with the same value type' do
    ...
    mismatched = english.filter_map do |path, value|
      next if missing.include?(path) || chinese.fetch(path).instance_of?(value.class)
  ...
  it 'preserves interpolation, markup, URL, and template tokens' do
    mismatched = english.filter_map do |path, value|
      ...
      chinese_tokens = chinese.fetch(path).scan(fragile_token_pattern).sort
      path.join('.') unless english_tokens == chinese_tokens
    end

    expect(mismatched).to be_empty
  end

  it 'provides Simplified Chinese Rails localization data' do
    expect(I18n.available_locales).to include(:zh)
    expect(I18n.t('language_name', locale: :zh)).to eq('简体中文')
