...
RSpec.describe 'Language setting', type: :request do
  ...
  it 'offers every shipped language on the general settings page' do
    ...
    expect(choices(response.body).map { |c| c.at_css('input[name="locale"]')['value'] }).to eq(%w[en de es fr pl])
  end
  ...
  it 'names each language in its own words' do
    ...
    expect(names).to eq(%w[English Deutsch Español Français Polski])
  end
