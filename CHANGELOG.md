...
- Geocoding settings now carry a requests-per-second limit, and Dawarich paces its lookups to stay inside it. Point-by-point reverse geocoding, place lookups, visit attribution and the search bar all draw from one shared queue per server, so the limit holds across every worker and web process on the instance rather than being multiplied by however many run at once. The rate is yours to set for your own Photon instance, Nominatim, Geoapify or LocationIQ — leave it blank for no limit. For the two hosted Photon servers Dawarich knows, it is enforced rather than suggested: komoot's public server is pinned to the 1 request/second it publishes, and ChibiGeo is capped at 25 (the fastest public plan) and starts on the free tier's 1, since asking for more than your plan allows only earns rejected lookups that show up as silently skipped geocodes. ENV-managed instances can set the same ceiling with `REVERSE_GEOCODING_RPS`. This replaces the previous komoot handling, which paused one second inside each job while the rest of the worker pool kept firing, and which only ever applied when komoot was configured through `PHOTON_API_HOST`. (#3343)
