...
- Days per Country moved off the map. It used to sit in the map's Create tab, next to the tools for making visits and areas, which is a strange place to keep a yearly report — you had to already know it was there. It now lives on the Insights page, next to the year selector, with the rest of the per-year numbers. The report itself is unchanged.
