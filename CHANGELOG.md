...
- The timeline got quieter. The day header is a single line of per-mode distance chips plus moving time; a track expands to a mode ribbon and only the legs that matter — stops render as separators, parking-lot noise collapses into a "transfer" chip, and low-confidence stretches say "moving" instead of guessing. The full segment list with corrections lives behind "All segments · edit".
