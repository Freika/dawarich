...
- Machine-detected visits are now regenerable: re-running detection replaces them wholesale while everything you touched (confirmed, renamed, re-placed, deleted) survives as-is. **Existing visits keep their old detection until re-detected: press the re-detect button in Settings → Visits to redo your own history.** Self-hosted instances start a fleet-wide re-detection automatically after migrating (it runs on the lowest-priority queue); Dawarich Cloud will be re-detected in a staged rollout.
