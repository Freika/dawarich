...
- Each user can now configure their own geocoding provider (Photon, Geoapify, Nominatim or LocationIQ) under Settings → Integrations, used for reverse geocoding points and places as well as place and location search. For Photon there is an explicit server choice — the ChibiGeo hosted service (default, with a free 2,500-lookups/day tier and a link to get an API key), komoot's public server (with a rate-limit warning), or a custom host — and HTTPS is locked on for hosted servers. Credentials are stored encrypted, one provider is active at a time, and a "Test my provider" button runs a live lookup against the saved configuration. The `PHOTON_API_HOST`/`GEOAPIFY_API_KEY`/`NOMINATIM_API_HOST`/`LOCATIONIQ_API_KEY` environment variables keep working and take precedence as instance-wide configuration; on self-hosted instances an existing ENV configuration is copied into every user's settings once, so removing the variables later keeps geocoding running uninterrupted.
- A custom basemap that covers only part of the world can now fall back to the built-in tiles for everything it leaves out. Point the map at a regional tileset — a single country's extract, say — and the areas beyond its coverage are drawn from the default basemap instead of being left blank. Turn it on with "Fill gaps with the default basemap", under Custom Basemap URL in the map's settings panel. It is off by default and stays off for existing setups, because switching it on means the browser starts requesting tiles from Dawarich's default tile server for the uncovered areas — which is precisely what some instances set a custom basemap to avoid. Where your own tiles do have data nothing changes: they are drawn over the default basemap, which only shows through the gaps. Both vector and raster `{z}/{x}/{y}` tile URLs are supported; a full MapLibre style URL replaces the entire basemap and so has nothing to fall back onto, and the option is disabled for those.
- Geocoding settings now carry a requests-per-second limit, and Dawarich paces its lookups to stay inside it. Point-by-point reverse geocoding, place lookups, visit attribution and the search bar are all spaced out to match, so a bulk backfill no longer fires a whole worker pool at a server that asked for one request a second. The rate is yours to set for your own Photon instance, Nominatim, Geoapify or LocationIQ - leave it blank for no limit. For the two hosted Photon servers Dawarich knows, it is enforced rather than suggested: komoot's public server is pinned to the 1 request/second it publishes, and ChibiGeo is capped at 25 (the fastest public plan) and starts on the free tier's 1, since asking for more than your plan allows only earns rejected lookups that show up as silently skipped geocodes. ENV-managed instances can set the same ceiling with `REVERSE_GEOCODING_RPS`. The reverse geocoding queue now holds at most a third of your Sidekiq threads, since a thread waiting on the rate limit adds no throughput and would otherwise keep imports and point ingestion waiting; raise `REVERSE_GEOCODING_CONCURRENCY` if your provider has no limit worth respecting. Pacing is counted per process, so the worker pool doing the bulk geocoding is held to the limit exactly; a separate web or worker process keeps its own count. This replaces the previous komoot handling, which paused one second inside each job while the rest of the worker pool kept firing, and which only ever applied when komoot was configured through `PHOTON_API_HOST`. (#3343)
- A new `point_sources` reference table deduplicates the device and importer metadata that was repeated on every point — on a 1.6M-point history the nine columns involved collapse to a couple of hundred rows. A resumable background backfill links existing points to it and resolves missing country references. Self-hosted instances start it automatically after migrating (to opt out, set `SKIP_POINT_DIMENSION_BACKFILL=1` before migrating, on every service including Sidekiq — the app and worker containers have separate environments); Dawarich Cloud is backfilled in a staged rollout. It walks the whole `points` table in paused batches and rewrites every row once, so on instances with millions of points expect it to run for hours in the background and to temporarily grow the database until autovacuum catches up. Nothing changes in the UI while it runs, and it can be re-run at any time to pick up rows it has not reached. Adding the linking column needs a brief exclusive lock on `points`; instances with heavy write traffic that cannot win it during startup hand the change to a background job so boot still completes, and the backfill starts once that job succeeds.
- Saving an AirTrail integration whose host is unreachable (connection refused, unresolvable hostname, TLS failure) no longer crashes the settings page — the settings are saved and the connection failure is reported in the flash message and the sidebar indicator instead. (#3393)
- Geocoding provider hosts are now checked against the same blocked-address list as the photo integrations, so a saved host can no longer point the server at link-local or cloud-metadata addresses. A host the web container cannot resolve can still be saved.
- Place and location search no longer stall a web request behind a busy geocoding rate limit: a lookup whose turn is more than a second away is skipped and returns no results instead of queueing. A skipped lookup is never cached as an empty answer, and the "Test my provider" button reports a busy rate limit instead of a failed connection.
- The geocoding "Test my provider" button no longer echoes internal error details for unexpected failures; connection errors keep their diagnostic message, everything else is logged instead.
- The nightly reverse-geocoding sweep and the realtime track pass resolve each user's geocoding settings once instead of once per point.


### Changed

- Tiled rendering (beta) now covers three more layers: Routes, Tracks and Fog of War all draw from vector tiles, so with tiles on only the Scratch map still needs the classic full download. Routes under tiled mode show your generated tracks — colored per track (speed coloring becomes a per-track average rather than a gradient along the line), with the route color and opacity settings still applying, while the route-splitting sliders have no effect and say so. An empty view shows a note instead of silence. Hovering a route no longer highlights it or opens the route popup — clicking a track opens its detail instead — and the separate below-zoom-8 unsplit route rendering is replaced by simplified track tiles at every zoom. Fog of War keeps working with the Points layer off. Deleted or reclassified tracks drop out of cached tiles automatically.
- The vector tile query timeout is now configurable via the `VECTOR_TILES_QUERY_TIMEOUT_MS` environment variable (default unchanged at 5000 ms). Self-hosted instances where very large date ranges exceeded the fixed 5-second bound — showing up as cancelled tile queries and terminated parallel workers in the PostgreSQL log — can raise it without a code change. (#3386)
- The Integrations settings page now shows one service at a time — Geocoding, Immich, PhotoPrism and AirTrail — chosen from a sidebar with each service's logo, instead of one long combined form. Every service in the sidebar carries a connection indicator: a green check when the last connection attempt succeeded, an orange alert when the service is configured but the last attempt failed, and nothing while it is not set up. Photo services record the result each time "Save & Test Connection" runs; the geocoding provider records it from its test button. Geocoding settings live here now instead of in their own settings tab, and the old address redirects. (#3393)
