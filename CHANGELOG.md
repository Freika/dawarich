...
- Geocoding provider hosts are now checked against the same blocked-address list as the photo integrations, so a saved host can no longer point the server at link-local or cloud-metadata addresses.
- Place and location search no longer stall a web request behind a busy geocoding rate limit: a lookup whose turn is more than a second away is skipped and returns no results instead of queueing.
...
- The nightly reverse-geocoding sweep resolves each user's geocoding settings once instead of once per point.
