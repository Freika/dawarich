...
- After upgrading, restart the app once migrations have run (the running process's schema cache doesn't pick up the new columns), and expect a background backfill to convert existing track segments to time-based anchoring — it runs on the lowest-priority queue and requires no action.
