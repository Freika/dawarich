...
- The transportation threshold sliders and expert mode are gone — the new detection has nothing to configure except which modes are enabled. Any saved threshold values are removed from settings and ignored by the settings API. **Existing tracks keep their old classification until re-detected: press Recalculate under Map settings → Transportation to redo your own tracks.** Self-hosted admins can redo every account at once from the console with `TransportationModes::FleetReclassifyJob.perform_later`.
