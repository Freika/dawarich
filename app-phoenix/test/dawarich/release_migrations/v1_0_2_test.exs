defmodule Dawarich.ReleaseMigrations.V1_0_2Test do
  ...
  test "20260125100000 without users.deleted_at rescues the users select and the import count" do
  ...
  test "20260125100000 keeps the user jobs when the select on the integer imports.source fails" do
    ...
    assert step().(ScratchRepo) ==
             {:jobs,
              [
                {"TransportationModes::BackfillJob", [1], 120},
                {"TransportationModes::BackfillJob", [3], 150}
              ]}
  end
