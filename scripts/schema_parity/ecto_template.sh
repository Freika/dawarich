...
build_template() {
  ...
  dexec "$db_container" psql -U postgres -qc "ALTER DATABASE $build_db SET timezone TO 'Pacific/Chatham'" >/dev/null
  case "$2" in
    none) ;;
  ...
  built_to="$(date -u +%s)"
  sleep 1
  dexec "$db_container" psql -U postgres -q -v ON_ERROR_STOP=1 -c "COMMENT ON DATABASE $build_db IS '$built_from $built_to'" \
    -c "ALTER DATABASE $build_db WITH IS_TEMPLATE true ALLOW_CONNECTIONS false" >/dev/null
  if dexec "$db_container" psql -U postgres -q -v ON_ERROR_STOP=1 -c "ALTER DATABASE $build_db RENAME TO $1" >/dev/null 2>&1; then
    build_db=""
  fi
}
...
prepare() {
  ...
  template_window="$(dexec "$db_container" psql -U postgres -qAt -v ON_ERROR_STOP=1 -c "CREATE DATABASE $1 TEMPLATE $template" \
...
prune_databases() {
  dexec "$db_container" psql -U postgres -qAt -c "SELECT datname FROM pg_database WHERE datname ~ '^sp_[tbre]_'" > "$work/ecto/.databases" || return 1
  ...
  drop_sql $(cat "$work/ecto/.databases.stale") | dexec -i "$db_container" psql -U postgres -q >/dev/null
}
