...
aggregate() {
  output="$(LEGS="$legs" PROVE_RESULT="${1:-success}" RUN_URL="$run_url" LANG=en_US.UTF-8 \
    ruby "$report" report "$scratch/artifacts" "$scratch/jobs.tsv" "$scratch/artifacts.tsv" 2>&1)"
  status=$?
}
...
contains "$output" "| 14 | 1 | success | 5 / 5 | 1:02:03 | 3 / 1 | 0 | 1 | [pg14-shard1]($run_url/artifacts/100) |"
verdict $? "the table has PG major, shard, job, checks run, elapsed, references, failures, unported and the artifact link"
contains "$output" "| 17 | 2 | success |" && contains "$output" "rows:1.7.6--unported-20260508193900"
verdict $? "every shard has a row and the declared unported effects are named"

all_green
leg pg14-shard2 1 "$(printf '%s\n' "$green" | sed 's/^upgrade:0.37.2 ok$/upgrade:0.37.2 FAIL schema:12 rows:3/')"
job_result pg14-shard2 'completed\tfailure'
aggregate
[ "$status" -eq 1 ]
verdict $? "one FAIL line fails the report (exit $status)"
contains "$output" "pg14-shard2: upgrade:0.37.2: expected 'ok', actual 'FAIL schema:12 rows:3'"
verdict $? "it names the check with its expected and actual status"
contains "$output" "pg14-shard2: the harness exited 1" && contains "$output" "pg14-shard2: job completed, failure"
verdict $? "it names the harness exit and the job result"

all_green
rm "$scratch/artifacts/pg17-shard1/ecto/summary.1-2.txt"
aggregate
[ "$status" -eq 1 ] && contains "$output" "pg17-shard1: no summary file" && contains "$output" "pg17-shard1: 5 of 5 checks have no summary line"
verdict $? "a missing summary fails the report and says so ($output)"

all_green
leg pg17-shard2 124 "$(printf '%s\n' 'fresh ok' 'ABORTED terminated while running the checks (unfinished: upgrade:0.37.2)')"
aggregate
[ "$status" -eq 1 ]
verdict $? "an ABORTED line fails the report (exit $status)"
contains "$output" "pg17-shard2: ABORTED terminated while running the checks (unfinished: upgrade:0.37.2)" \
  && contains "$output" "pg17-shard2: the harness timed out" && contains "$output" "pg17-shard2: 4 of 5 checks have no summary line"
...
all_green
leg pg14-shard2 0 "$(printf '%s\n' "$green" | sed 's/ ok (failed@20260827200000)$/ ok/')"
aggregate
...
all_green
leg pg17-shard1 0 "$(printf '%s\n' "$green" 'fresh ok' 'step:9.9.9 ok')"
aggregate
...
status=$?
[ "$status" -eq 0 ] && contains "$output" "5 of 5 checks, 1 unported"
verdict $? "leg mode passes a green shard ($output)"
leg pg14-shard1 1 "$(printf '%s\n' "$green" | sed 's/^fresh ok$/fresh FAIL ledger:1/')"
output="$(LANG=en_US.UTF-8 ruby "$report" leg "$scratch/artifacts/pg14-shard1" 2>&1)"
status=$?
