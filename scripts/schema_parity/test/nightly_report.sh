...
contains "$output" "| 14 | 1 | success | 3 / 3 | 1:02:03 | 3 / 1 | 0 | [pg14-shard1]($run_url/artifacts/100) |"
verdict $? "the table has PG major, shard, job, checks run, elapsed, references, failures and the artifact link"
contains "$output" "| 17 | 2 | success | 2 / 2 |"
verdict $? "every shard has a row"
...
leg pg17-shard2 124 "$(printf '%s\n' 'contended:1.14.1:points ok (failed@20260827200000)' \
  'ABORTED terminated while running the checks (unfinished: rows:1.7.6--duplicate-tracks)')"
aggregate
[ "$status" -eq 1 ]
verdict $? "an ABORTED line fails the report (exit $status)"
contains "$output" "pg17-shard2: ABORTED terminated while running the checks (unfinished: rows:1.7.6--duplicate-tracks)" \
...
status=$?
[ "$status" -eq 0 ] && contains "$output" "2 of 2 checks, references 3 reused / 1 computed, 0 problems"
