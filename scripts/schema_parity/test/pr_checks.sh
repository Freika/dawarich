...
cp "$root/scripts/schema_parity/pr_checks.rb" "$root/scripts/schema_parity/inventory_tags.rb" \
  "$root/scripts/schema_parity/inline_effects.tsv" "$repo/scripts/schema_parity/"
...
reset_tree
edit "$counterparts" '1,/uses: actions\/checkout@/s|actions/checkout@[0-9a-f]\{40\} # v7.0.1|actions/cache@55cc8345863c7cc4c66a329aec7e433d2d1c52a9 # v6.1.0|'
commit_case
expect "$full" "a uses: line that swaps one action for another selects every check" "$base...$head"

reset_tree
bump_pin "$counterparts"
printf '      - uses: actions/setup-node@0123456789abcdef0123456789abcdef01234567 # v4.0.0\n' >> "$repo/$counterparts"
commit_case
expect "$full" "a pin bump next to an added uses: line selects every check" "$base...$head"

reset_tree
edit "$counterparts" '/uses: actions\/upload-artifact@/d'
commit_case
expect "$full" "removing uses: lines selects every check" "$base...$head"
