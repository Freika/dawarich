...
check('the proved list is the sample, with one ok line per check, even after a failure') do
...
check("every Mix cache here and in the nightly uses the mix-parity key, separate from ci.yml's") do
  key = "mix-parity-${{ runner.os }}-${{ hashFiles('app-phoenix/.tool-versions', 'app-phoenix/mix.lock') }}"
  caches = [text, nightly].flat_map { Psych.safe_load(_1).fetch('jobs').values.flat_map { |job| job['steps'] } }
                          .select { _1['name'] == 'Cache Mix' }
  caches.size == 3 && caches.all? { _1.dig('with', 'key') == key }
end
