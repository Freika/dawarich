...
prove_leg
sed -i.bak "2s/ ok\$/ ok (failed@20260720170000)/" "$leg/ecto/summary.txt"
output="$(verify_leg)"
[ $? -ne 0 ] && contains "$output" "upgrade:$latest: expected 'ok', actual 'ok (failed@20260720170000)'"
verdict $? "a result other than plain ok on a sampled upgrade fails the summary check ($output)"
