...
export class VisitsManager {
  constructor(map, apiKey, userTheme = "dark", mapsController = null) {
    ...
    this.timezone =
      ...
      mapsController?.userSettings?.timezone ||
      "UTC"
  ...
  displayVisits(visits) {
    ...
    const visitsHtml = visits
      ...
      .map((visit) => {
        ...
        const isSameDay =
          startDate.toLocaleDateString(undefined, tzOptions) ===
          endDate.toLocaleDateString(undefined, tzOptions)
  ...
  async mergeVisits(visitIds) {
    if (!visitIds || visitIds.length < 2) {
      showFlashMessage(
        "error",
        "At least 2 visits must be selected for merging",
      )
      return
    }
