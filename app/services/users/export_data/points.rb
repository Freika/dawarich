...
class Users::ExportData::Points
  ...
  def build_full_query
    <<-SQL
      SELECT
        p.id, ps.battery_status, p.battery, p.timestamp, p.altitude, p.velocity, p.accuracy,
        ps.tracker_id, ps.topic, ps.trigger, ps.bssid, ps.ssid,
        ps.connection,
        p.vertical_accuracy, ps.inrids, ps.in_regions, p.raw_data,
        p.city, p.geodata, p.reverse_geocoded_at, p.course,
        p.course_accuracy, p.created_at, p.updated_at,
        p.lonlat,
